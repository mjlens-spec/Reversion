...
当前版本：**2.1.3**（Apple Silicon）。反文沿用 MarkText 的应用数据目录与 Bundle ID，因此从旧版本迁移时设置、历史记录与自动更新链均可延续。
...
- 侧栏启动时恢复为约定的 `1 : 2.34` 侧栏与编辑区比例；当前会话仍可手动拖动调整。
- 正文文字区域动态占可读区域的 76%，不同窗口尺寸下左右各保留 12%。
- 全新配置默认使用 Claude-like；已有配置中用户主动选择的主题保持不变。

## 大版本记录

- **2.1.0** — 中文排印、阅读参数、专注与打字机模式、明暗外观，以及当前 App Icon。
- **2.0.0** — 文档优先的页面形态、语义缩略图、独立中西文字体与双语命令面板。
- **1.9.0** — 文件名优先的标题栏与紧凑路径标签。
- **1.8.0** — 面向单文件和项目的目录树。
- **1.7.0** — 工作区外壳、常驻标签、导出入口与划选工具条整体重做。
- **1.6.0** — 新增 DOCX 与 PNG 长图导出。
- **1.5.0** — 完善表格与链接编辑。
- **1.3.0** — 迁移至 TypeScript Muya 引擎，并补齐应用内更新流程。

完整说明与下载见 [Releases](https://github.com/mjlens-spec/Reversion/releases)。

## 开发计划

**后续。** 行内实时渲染的行为对齐 Typora（光标锚定、点击落点、链接编辑）、表格编辑（行列拖拽手柄、Excel/CSV 粘贴成表、宽表格滚动），以及 PDF 导出的大纲书签与分页控制。

少数输入法边缘用例连同复现说明记录在 `tests-e2e/helpers/known-issues.ts`。
