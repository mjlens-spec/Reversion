...
当前版本：**2.1.2**（Apple Silicon）。反文沿用 MarkText 的应用数据目录与 Bundle ID，因此从旧版本迁移时设置、历史记录与自动更新链均可延续。
...
- 长文末尾收敛为 64 px 留白，打字机模式仍保留输入居中空间。
- 宽窗口下正文区域更舒展，窄窗口仍会自适应，不产生横向溢出。
- 编辑器与样式化导出的表格字号统一比正文小 2 px。
