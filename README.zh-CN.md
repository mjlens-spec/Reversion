...
当前版本：**1.9.1**（Apple Silicon）。反文沿用 MarkText 的应用数据目录与 Bundle ID，因此从旧版本迁移时设置、历史记录与自动更新链均可延续。
...
- **工作区外壳与写作区域重新对齐。** 左侧目录略微上移，分栏按钮移至侧栏右上角，标签页移至正文页底部。
- **横线与标题建立统一基准。** 左侧分隔线与正文页顶部横线处于同一高度，当前文档标题在正文区域内水平居中，并调整到合适的垂直位置。
- **原有交互保持不变。** 标签切换、侧栏开关、导出、文件名重命名和未保存状态提示继续沿用原有行为。
...
完整说明见 [Releases](https://github.com/mjlens-spec/Reversion/releases/tag/v1.9.1)。
