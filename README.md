...
Current release: **1.9.1** (Apple Silicon). Reversion keeps MarkText's application data directory and bundle identifier, so preferences, history, and updater continuity survive migration from earlier versions.
...
- **The workspace chrome now follows the writing surface.** The sidebar TOC sits slightly higher, the sidebar toggle is in the sidebar's upper-right corner, and the tabs live at the bottom of the editor pane.
- **The header line and title share one alignment.** The sidebar divider and editor header line meet at the same height, while the active document title is centered within the editor pane.
- **Existing controls remain intact.** Tab switching, sidebar toggling, export, rename behavior and the document state indicator continue to use their existing interactions.
...
Full notes: [Releases](https://github.com/mjlens-spec/Reversion/releases/tag/v1.9.1).
...
```bash
./scripts/build-release-from-source.sh 1.9.1
```
